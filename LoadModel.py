...
print("ad_act_test len: ",len(ad_act_test))
...
print("doc_pos_y:", doc_pos_y[0], ", len: ", len(doc_pos_y), ", d: ", len(doc_pos_y[0]))
...
with open(file, 'a+') as f:
    ...
    for i in range(len(doc_pos_y)):
        ...
        for j in doc_pos_y[i].tolist():
            ...
            index += 1
        # print()
        line=ad_act_test[i] + "\t" + str(query_norm_single[i][0]) + "\t" + ",".join(s)
        f.write(line + '\n')  # 加\n换行显示